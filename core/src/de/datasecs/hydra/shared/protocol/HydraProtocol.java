...
public class HydraProtocol {
    ...
    public void callPacketListener(Packet packet, Session session) {
        try {
            Method c = packetListenerMethods.get(packet.getClass().getSuperclass());
            if (c != null) {
            	c.invoke(packetListener, packet.getClass().cast(packet), session);
            } else {
            	GameU.log("key: "+packet.getClass().toString());
            	for (Entry<Class<?>, Method> e : packetListenerMethods.entrySet()) {
            		GameU.log("entry: "+e.getKey().toString()+" "+e.getValue().toString());
            	}
            	GameU.end("shit!");
            }
